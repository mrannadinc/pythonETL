...
def main():
    ...
    try:
        ...
        configfile = r"/home/kaan.mutlu/VizAlerts-master/config/vizalerts.yaml"
        ...
        if not len(log.logger.handlers):
            log.logger = log.LoggerQuickSetup(config.configs[r"log.dir"] + 'vizalerts.log', log_level=config.configs['log.level'])
    except Exception as e:
    ...
    try:
        cleanup_dir(config.configs[r"temp.dir"], config.configs['temp.dir.file_retention_seconds'])
    except OSError as e:
        # Send mail to the admin informing them of the problem, but don't quit
        errormessage = 'OSError: Unable to cleanup temp directory {}, error: {}'.format(config.configs[r"temp.dir"], e)
        log.logger.error(errormessage)
        ...
        errormessage = 'Unable to cleanup temp directory {}, error: {}'.format(config.configs[r"temp.dir"], e)
        log.logger.error(errormessage)
        email_instance = emailaction.Email(config.configs['smtp.address.from'], config.configs['smtp.address.to'], config.configs['smtp.subject'], errormessage)
        emailaction.send_email(email_instance)
    ...
    try:
        cleanup_dir(config.configs[r"log.dir"], config.configs['log.dir.file_retention_seconds'])
    except OSError as e:
        # Send mail to the admin informing them of the problem, but don't quit
        errormessage = 'OSError: Unable to cleanup log directory {}, error: {}'.format(config.configs[r"log.dir"], e)
        log.logger.error(errormessage)
        ...
        errormessage = 'Unable to cleanup log directory {}, error: {}'.format(config.configs[r"log.dir"], e)
        log.logger.error(errormessage)
        emailaction.send_email(config.configs['smtp.address.from'], config.configs['smtp.address.to'], config.configs['smtp.subject'], errormessage)
    ...
    try:
        csv_to_excel(PATH_CSV, PATH_EXCEL)
        log.logger.info('Converting CSV file to EXCEL file')
    except Exception as e:
        errormessage = 'Unable to convert please check the path and other parameters'
        log.logger.error(errormessage)
        quit_script(errormessage)
...
def get_alerts():
    ...
    statefile = config.configs[r"schedule.state.dir"] + SCHEDULE_STATE_FILENAME
...
def csv_to_excel(path_csv, path_excel):
        read_file = pd.read_csv(path_csv)
        read_file.to_excel(path_excel, index=None, header= True, encoding='utf-8')
