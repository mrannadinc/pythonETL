...
class Transform:
    ...
    def write_personalreport_data(self):
        ...
        for chunk in pd.read_sql_query("SELECT ID,"
                                       ...
                                       "GENDER,"
                                       "SERVICEUNIT,"
                                       "USERTYPE FROM personalreport;", self.windesk_mysql_connection, chunksize=10000):
...
def main():
    ...
    issuereport_start_time = time.time()
    transform_data.write_issuereport_data()
    issuereport_end_time = time.time()
    print("issuereport Transform Completed with(time) : ", issuereport_end_time - issuereport_start_time)
    callreport_start_time = time.time()
    transform_data.write_callreport_data()
    callreport_end_time = time.time()
    print("callreport Transform Completed with(time) : ", callreport_end_time - callreport_start_time)
    personalreport_start_time = time.time()
    transform_data.write_personalreport_data()
    personalreport_end_time = time.time()
    print("personalreport Transform Completed with(time) : ", personalreport_end_time - personalreport_start_time)
